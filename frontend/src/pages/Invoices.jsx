...
import { listInvoices, getInvoiceStats } from "@/lib/invoices";
...
function StatCard({ Icon, label, value, foot, tone = "emerald", secret, revealed, onToggleSecret, revealLabel, hideLabel }) {
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-2 flex items-center gap-2">
            <p className="truncate text-2xl font-bold text-text">{secret && !revealed ? MASK : value}</p>
            {secret && (
...
export default function Invoices() {
  ...
  // Money is masked ("***") on every load; each toggle is opt-in per session.
  const [showMoney, setShowMoney] = useState(false); // stats-card money
  const [showTotals, setShowTotals] = useState(false); // table total column
  ...
  }, [q, by, dateFrom, dateTo, page, t, toast]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        {canModify && (
          <button onClick={() => setModal({ open: true, mode: "create", invoice: null })}
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard tone="amber" Icon={IconWallet} label={t("invoices.stats.revenue")}
          value={stats ? money(stats.overall.revenue) : "—"}
          foot={stats && monthFoot(showMoney ? money(stats.month.revenue) : MASK)}
          secret revealed={showMoney} onToggleSecret={() => setShowMoney((v) => !v)}
          revealLabel={t("invoices.show")} hideLabel={t("invoices.hide")} />
        <StatCard tone="emerald" Icon={IconReceipt} label={t("invoices.stats.invoices")}
          value={stats?.overall.invoices ?? "—"}
          foot={stats && monthFoot(stats.month.invoices)} />
        <StatCard tone="sky" Icon={IconList} label={t("invoices.stats.items")}
          value={stats?.overall.items ?? "—"}
          foot={stats && monthFoot(stats.month.items)} />
        <StatCard tone="violet" Icon={IconDiscount} label={t("invoices.stats.discount")}
          value={stats ? money(stats.overall.discount) : "—"}
          foot={stats && monthFoot(showMoney ? money(stats.month.discount) : MASK)}
          secret revealed={showMoney} onToggleSecret={() => setShowMoney((v) => !v)}
          revealLabel={t("invoices.show")} hideLabel={t("invoices.hide")} />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2 text-sm">
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)}
            className="ctrl-input-sm" title={t("invoices.from")} />
          <span className="text-muted">–</span>
          <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)}
            className="ctrl-input-sm" title={t("invoices.to")} />
        </div>
      </div>
            ...
            <table className="ctrl-table w-full border-collapse text-sm">
              <thead className="sticky top-0 z-10 bg-surface">
                <tr className="border-b border-border text-xs uppercase tracking-wide text-muted">
                  ...
                  <th className="px-4 py-3 text-center font-medium">{t("invoices.table.items")}</th>
                  <th className="px-4 py-3 text-end font-medium">
                    <span className="inline-flex items-center gap-1.5">
                      {t("invoices.table.total")}
                      <button type="button" onClick={() => setShowTotals((v) => !v)}
                        title={showTotals ? t("invoices.hide") : t("invoices.show")}
                        className="text-muted transition hover:text-text">
                        {showTotals ? <IconEyeOff width={15} height={15} /> : <IconEye width={15} height={15} />}
                      </button>
                    </span>
                  </th>
                  {canModify && <th className="px-4 py-3 text-end font-medium">{t("invoices.table.actions")}</th>}
                </tr>
              </thead>
              <tbody>
                {data.items.map((s) => {
                  ...
                  return (
                    <FragmentRow key={s.id} s={s} isOpen={isOpen} cols={canModify ? COLS + 1 : COLS}
                      canModify={canModify} showTotals={showTotals} onToggle={() => toggle(s.id)}
                      onEdit={() => setModal({ open: true, mode: "edit", invoice: s })}
                      money={money} fmtDate={fmtDate} isAr={isAr} t={t} iconBtn={iconBtn} />
                  );
                })}
              </tbody>
            </table>
...
function FragmentRow({ s, isOpen, cols, canModify, showTotals, onToggle, onEdit, money, fmtDate, isAr, t, iconBtn }) {
  return (
    <>
      <tr className={`border-b border-border/60 transition hover:bg-elevated/40 ${isOpen ? "bg-elevated/30" : ""}`}>
        ...
        <td className="cursor-pointer px-4 py-3" onClick={onToggle}>
          <span className="inline-flex flex-wrap items-center gap-2">
            <span className="font-mono font-semibold text-accent" dir="ltr">{s.invoice_no}</span>
            {s.is_backtrack && (
        ...
        <td className="px-4 py-3 text-text">
          ...
          {s.customer_phone && <span className="ms-1 text-xs text-muted" dir="ltr">({s.customer_phone})</span>}
        </td>
        ...
        <td className="px-4 py-3 text-center">
          <span className="rounded-full bg-accent/15 px-2.5 py-0.5 text-xs font-semibold text-accent">{s.item_count}</span>
        </td>
        <td className="px-4 py-3 text-end font-medium text-text tabular-nums">{showTotals ? money(s.total) : MASK}</td>
        {canModify && (
          ...
          <td colSpan={cols} className="px-4 pb-4 pt-1">
                    ...
                    <tr key={idx} className="border-t border-border/60">
                      <td className="px-3 py-2">
                        ...
                        <div className="mt-0.5 flex flex-wrap items-center gap-1.5">
                          <span className="font-mono text-[11px] text-muted" dir="ltr">{it.code}</span>
                          {(it.attributes || []).map((a, i) => (
                            <span key={i} className="inline-flex items-center gap-1 rounded-full bg-elevated px-2 py-0.5 text-[11px] text-muted">
                              {a.hex && <span className="h-2.5 w-2.5 rounded-full border border-border" style={{ backgroundColor: a.hex }} />}
                      ...
                      <td className="px-3 py-2 text-end font-medium tabular-nums text-text">{money(it.line_total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="mt-3 ms-auto w-full max-w-xs space-y-1 text-sm">
              <div className="flex justify-between text-muted">
                <span>{t("invoices.modal.subtotal")}</span><span className="tabular-nums">{money(s.subtotal)}</span>
              </div>
              <div className="flex justify-between text-muted">
                <span>{t("invoices.modal.discount")}</span><span className="tabular-nums">{money(s.discount)}</span>
              </div>
              <div className="flex justify-between border-t border-border pt-1 font-semibold text-text">
                <span>{t("invoices.modal.total")}</span><span className="tabular-nums">{money(s.total)}</span>
              </div>
            </div>
          </td>
        </tr>
